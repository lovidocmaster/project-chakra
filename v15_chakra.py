...
# ── Infrastructure wrappers (v13 uses different internal names) ───
...
def get_oanda_candles(pair, granularity="H1", count=100):
    try:
        from oandapyV20 import API as _OA
        from oandapyV20.endpoints.instruments import InstrumentsCandles as _IC
        api = _OA(access_token=OANDA_TOKEN, environment=OANDA_ENV)
        ep  = _IC(pair, params={"count": count, "granularity": granularity})
        api.request(ep)
        return ep.response.get("candles", [])
    except Exception:
        return []

def place_oanda_order(pair, units, sl, tp):
    try:
        from oandapyV20 import API as _OA
        from oandapyV20.endpoints.orders import OrderCreate as _OC
        prec = 3 if "JPY" in pair else (2 if "XAU" in pair else 5)
        fmt  = f"{{:.{prec}f}}"
        api  = _OA(access_token=OANDA_TOKEN, environment=OANDA_ENV)
        data = {"order": {"type": "MARKET", "instrument": pair,
                          "units": str(units), "timeInForce": "FOK",
                          "stopLossOnFill":   {"price": fmt.format(sl)},
                          "takeProfitOnFill": {"price": fmt.format(tp)},
                          "positionFill": "DEFAULT"}}
        r = _OC(OANDA_ACCOUNT, data=data)
        api.request(r)
        return r.response
...
def get_open_positions():
    try:
        from oandapyV20 import API as _OA
        from oandapyV20.endpoints.trades import OpenTrades as _OT
        api = _OA(access_token=OANDA_TOKEN, environment=OANDA_ENV)
        r   = _OT(OANDA_ACCOUNT)
        api.request(r)
...
def log_to_supabase(table, record):
    try:
...
class H4TrendFilter:
    ...
    def allows(self,pair,direction,h4_candles=None):
        ...
        if t=="BULLISH" and direction=="BUY": return True,"H4 BULLISH OK"
        if t=="BEARISH" and direction=="SELL": return True,"H4 BEARISH OK"
...
class V15Orchestrator:
    ...
    def prefetch_candles(self):
        ...
        log.info(f"[Cache] Fetched {len(PAIRS)} pairs x {len(grans)} TFs")
    ...
    def analyze_pair(self,pair):
        try:
            ...
            def convert(candles):
                ...
                for c in candles:
                    ...
                    result.append(SimpleNamespace(
                        ...
                        volume=0,mid=c["mid"]))
                return result
            cm15=convert(m15); ch1=convert(h1); ch4=convert(h4)
            regime=self.regime_det.detect(ch1) if ch1 else "RANGING"
            ...
            def run_agent(ag):
                try:
                    s=ag.analyze(pair,cm15,ch1,ch4)
                    return ag.name,s
